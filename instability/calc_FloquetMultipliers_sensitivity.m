...
for ne = 1:NEXP
    ...
    % shear = Shear_parm(ne); 
    % expname = ['H1500_topo4_Pt43200_N0.001_S' num2str(shear) '_lambda1000_dz' num2str(dz_group(ne)) '_dt' num2str(dt_group(ne))];
...
clf;
set(gcf,'Color','w')
plot(N_parm,log10_muk_buoy,'LineWidth',2)
hold on;
plot(N_parm,log10_muk_zeta,'LineWidth',2)
plot(N_parm,log10_muk_psi,'LineWidth',2)
% plot(topo_parm,zeros(1,NEXP),'k--','LineWidth',1)
set(gca,'Fontsize',fontsize)
grid on;grid minor;
title('Maximum Floquet exponents log(\mu) (\lambda = 1000 m)','Fontsize',fontsize+5)
