...
for o = 2:nR 
    ...
    for k=1:ncycle
        ...
        elseif(ntransition(o)==6)
            ...
            eq1 = a1*cos(o1*((k-1)*2*pi+t1)+p1)-a2*cos(o4*((k-1)*2*pi+t1)+p2)==0;
            eq2 = o1*a1*sin(o1*((k-1)*2*pi+t1)+p1)-o4*a2*sin(o4*((k-1)*2*pi+t1)+p2)==0;
            S = solve(eq1,eq2);
            p2 = double(S.p2(nd));
            a2 =double(S.a2(nd));
            
            eq3 = a2*cos(o4*((k-1)*2*pi+t2)+p2)-a3*cos(o1*((k-1)*2*pi+t2)+p3)==0;
            eq4 = o4*a2*sin(o4*((k-1)*2*pi+t2)+p2)-o1*a3*sin(o1*((k-1)*2*pi+t2)+p3)==0;
            S = solve(eq3,eq4);
            p3 = double(S.p3(nd));
            a3 =double(S.a3(nd));
            ...
            plot(k-1+tt2/2/pi,z2,'LineWidth',2,'Color',brown)
...
grow_rw_max = max(grow_rw,[],2);

plot(1./Ri_km,grow_mzero,'--','LineWidth',2)
plot(1./Ri_km,grow_rw_max,':','LineWidth',2)
xlim([0 4.4])
ylim([-0.04 0.35])
