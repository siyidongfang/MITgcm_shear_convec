...
plot_mitgcm_parm = [0.6:0.1:1.3]*1e-3;
...
clf;
set(gcf,'Color','w')
p1 = plot(plot_parm,log10_muk_buoy,'LineWidth',2);
% p1 = plot(plot_parm,log10_muk_zeta,'LineWidth',2);
hold on;
plot(plot_parm,ones(1,NEXP),'k--','LineWidth',1)
s1 = scatter(plot_mitgcm_parm(1:4),gr_tt(1:4),100,'Filled');
s1 = scatter(plot_mitgcm_parm(5:8),gr_tt(5:8),100,'Filled');

% s1 = scatter(plot_mitgcm_parm(1:4),gr_uu(1:4),100,'Filled');
% s1 = scatter(plot_mitgcm_parm(5:8),gr_uu(5:8),100,'Filled');
set(gca,'Fontsize',fontsize)
grid on;grid minor;
title('Mean Floquet exponents \mu (\lambda = 400 m)','Fontsize',fontsize+5)
xlabel('Velocity shear \Lambda (1/s)','Fontsize',fontsize+5)
legend([p1,s1],'Floquet \mu_k^b', '\mu_k^\theta of MITgcm','Fontsize',fontsize+5,'Position',[0.2000 0.6655 0.1676 0.1747])
